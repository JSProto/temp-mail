...
class TempMail {
     ...
     */
    getAvailableDomains() {
        return this.crawl('/option/change/', true).then($ => {
    ...
    change(mail, domain = this.getRandomDomain()) {
        ...
        return needle('post', `${this.host}/option/change/`, data, opts).then(res => {
            this.options.cookies = res.cookies;
            console.log('change mail: ', res.cookies.mail);
    ...
    remove(mail = '') {
        ...
        return this.crawl('/option/delete/', false).then(($) => {
     ...
     */
    refresh() {
        return this.crawl('/option/refresh/', false).then(($) => {
            this.email = $('#mail').val();
            console.log('check inbox: ', this.email);
     ...
     */
    view(hash){
        return this.crawl(`/view/${hash}`, false).then($ => {
    ...
    delete(hash){
        this.crawl(`/delete/${hash}`, false).then($ => {
            if (hash in this.inbox) {
    ...
    source(hash){
        this.crawl(`/source/${hash}`, false).then($ => {
            let body = $('.pm-text').text();
            console.log(body.trim());
        });
    }
